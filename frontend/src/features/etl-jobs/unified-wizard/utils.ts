...
export function buildJobPayload(state: WizardState): ETLJobCreate {
  ...
  const sourceConfig = state.sourceType === 'google_sheets'
...
/**
 * Auto-populate wizard state from Google Sheets selection
 */
export function autoPopulateFromGoogleSheets(
  ...
  const columnMappings: ColumnMappingConfig[] = columns.map((col, index) => ({
    sourceColumn: col,
    destinationColumn: col, // Auto-map to same name initially
    sourceType: 'TEXT', // Default to TEXT, user can change in column mapping
  ...
  // Create detected columns
  const detectedColumns = columns.map((col, idx) => ({
    name: col,
    type: 'string',
    order: idx,
  }))

  // Use sheet name as job name base
  const jobName = sheetName.replace(/[^a-zA-Z0-9_]/g, '_')
...
export function remapColumnsToTable(
  ...
  // Auto-generated columns that are managed by the backend
  ...
  const unmappedTableColumns: ColumnMappingConfig[] = tableColumns
    .filter(tableCol => {
      ...
      // Skip auto-generated timestamp columns - these are managed automatically by the backend
      if (autoGeneratedColumns.has(lowerName)) {
        return false
      }
