...
export function ColumnMappingGrid({
  ...
  // For new tables or truncate_insert, allow editing column types
  const canEditColumnTypes = isCreatingNewTable || loadStrategy === 'truncate_insert'
  ...
  // Separate columns into source-mapped and table-only
  // Exclude auto-generated timestamp columns from source columns
  const sourceColumns = columnMappings
    .filter(col => col.sourceColumn)
    .filter(col => !autoGeneratedColumns.has(col.destinationColumn?.toLowerCase() || ''))

  // Filter table-only columns to remove any that are already mapped from source columns
  // This handles the case where user manually maps a source column after the initial remapping
  const sourceMappedColumnNames = new Set(
    sourceColumns
  ...
  // 2. Have defaults
  // 3. Are nullable
  // Don't show NOT NULL columns with no defaults - those should be mapped from source columns
  // Also exclude auto-generated timestamp columns
  const tableOnlyColumns = columnMappings
    .filter(col => !col.sourceColumn)
    .filter(col => !sourceMappedColumnNames.has(col.destinationColumn?.toLowerCase() || ''))
    .filter(col => !autoGeneratedColumns.has(col.destinationColumn?.toLowerCase() || ''))
    .filter(col => {
      ...
      // Otherwise, don't show - it should be mapped from a source column
      return false
    })

  // Combine all mappings for unified display: source columns first, then table-only columns
  const allMappings = [...sourceColumns, ...tableOnlyColumns]
  ...
  // Handle mapping a source column to a DB column
  const handleMapColumn = (sourceIndex: number, dbColumnName: string) => {
    let updated = [...columnMappings]
    const sourceMapping = sourceColumns[sourceIndex]
    ...
    const fullIndex = columnMappings.findIndex(m =>
      m.sourceColumn === sourceMapping.sourceColumn &&
      m.columnOrder === sourceMapping.columnOrder
    )

    // Update the source column mapping
    updated[fullIndex] = {
      ...sourceMapping,
    ...
    updated = updated.filter((col, idx) => {
      // Keep all source columns (those with sourceColumn)
  ...
  // Handle unmapping (exclude) a source column
  const handleUnmapColumn = (sourceIndex: number) => {
    let updated = [...columnMappings]
    const sourceMapping = sourceColumns[sourceIndex]
    const previousDestination = sourceMapping.destinationColumn
  ...
  // Handle toggling transformations for a source column
  const handleToggleTransformation = (sourceIndex: number, transformationName: string) => {
  ...
  const getUnmappedRequiredColumns = () => {
    ...
    return tableSchema
      .filter(tableCol => {
        ...
        // Check if this column is mapped from a source column
        const isMappedFromSource = sourceColumns.some(sourceCol =>
          sourceCol.destinationColumn === tableCol.name && !sourceCol.exclude
        )
        if (isMappedFromSource) return false // It's mapped, no action needed
    ...
    <div className="space-y-6">
      ...
      {unmappedRequiredColumnNames.length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            ...
            Please map {unmappedRequiredColumnNames.length === 1 ? 'it' : 'them'} to source columns.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Map source columns to database table columns. Database columns without a source are shown with their status.
          </CardDescription>
        </CardHeader>
        <CardContent>
                        ...
                        <>
                          {!mapping.exclude && mapping.destinationColumn ? (
                            <>
                              ...
                              {/* Primary Key Checkbox with Badge */}
                              <div className="flex items-center gap-2">
                                <Checkbox
                                  id={`pk-${idx}`}
                                ...
                                />
                                <Label htmlFor={`pk-${idx}`} className="text-sm cursor-pointer">
                                  Primary Key
                                </Label>
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (tableOnlyIdx !== -1) {
                                  ...
                                  const allMappings = [...sourceColumns, ...updatedTableOnly]
